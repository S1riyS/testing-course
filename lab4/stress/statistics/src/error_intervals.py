...
def analyze_error_cascade():
    ...
    error_intervals = error_intervals.dropna()
    error_intervals = error_intervals[error_intervals >= 0]
    ...
    if len(error_intervals) > 10:
        ...
        plt.plot(rolling_mean, color='red',
                 label=f'{window_size}-Error Moving Average')
        plt.title('Error Intervals')
